...
const Dweet = require('../dweets/dweet.model');
...
exports.getProfiles = async (req, res) => {
  ...
  const filters = pick(req.query, ['following', 'followers', 'likes', 'redweets']);
  ...
  if (filters.likes && !(await Dweet.exists({ _id: filters.likes }))) {
    throw new ErrorHandler(404, 'Dweet does not exists');
  }

  if (filters.redweets && !(await Dweet.exists({ _id: filters.redweets }))) {
    throw new ErrorHandler(404, 'Dweet does not exists');
  }

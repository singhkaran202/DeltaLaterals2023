...
describe('Users routes', () => {
  ...
  describe('DELETE /api/users/:userId', () => {
    it('When owner is deleting, should delete user, their profile and dweets, and return 200', async () => {
      await insertUsers([userOne]);
      await Profile.create({ user: userOne._id });
      await Dweet.insertMany([
        {
          author: userOne._id,
      ...
      const [dbUser, dbProfile, dbDweets] = await Promise.all([
        User.findById(userOne._id),
        Profile.findOne({ user: userOne._id }),
        Dweet.find({ author: userOne._id }),
      ]);

      expect(dbUser).toBeNull();
      expect(dbProfile).toBeNull();
      expect(dbDweets).toHaveLength(0);
    });

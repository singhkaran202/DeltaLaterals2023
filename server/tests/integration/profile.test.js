...
const Dweet = require('../../src/components/dweets/dweet.model');
...
describe('Profiles routes', () => {
  describe('GET /api/profiles', () => {
    ...
    it('When likes filter is applied, should return profiles who liked a dweet', async () => {
      const dweetId = mongoose.Types.ObjectId();
      await Promise.all([
        insertUsers([userOne, userTwo]),
        Profile.insertMany([{ user: userOne._id, likes: [dweetId] }, { user: userTwo._id }]),
        Dweet.insertMany([
          {
            _id: dweetId,
            author: userTwo._id,
      ...
      const res = await request(app).get('/api/profiles').query({ likes: dweetId.toString() });
    ...
    it('When redweets filter is applied, should return profiles who redweeted a dweet', async () => {
      const dweetId = mongoose.Types.ObjectId();
      await Promise.all([
        insertUsers([userOne, userTwo]),
        Profile.insertMany([{ user: userOne._id, redweets: [dweetId] }, { user: userTwo._id }]),
        Dweet.insertMany([
          {
            _id: dweetId,
            author: userTwo._id,
            text: faker.random.words(10),
            redweets: [userOne._id],
          },
        ]),
      ]);

      const res = await request(app).get('/api/profiles').query({ redweets: dweetId.toString() });
    ...
    it('When likes filter is applied and dweet does not exists, should return 404 error', async () => {
      const dweetId = mongoose.Types.ObjectId();
      await Promise.all([insertUsers([userOne]), Profile.insertMany([{ user: userOne._id, likes: [dweetId] }])]);
    ...
    it('When redweets filter is applied and dweet does not exists, should return 404 error', async () => {
      const dweetId = mongoose.Types.ObjectId();
      await Promise.all([insertUsers([userOne]), Profile.insertMany([{ user: userOne._id, redweets: [dweetId] }])]);
